...
def get_mecanico_by_id(mecanico_id: int):
    ...
    query = """
    SELECT id_usuario, nome, especialidade, telefone, email, ativo,
...
def create_mecanico(mecanico: Mecanico):
    ...
    mecanico.id_usuario = mecanico_id
    logger.info("mecânico criado id=%s nome=%s", mecanico.id_usuario, mecanico.nome)
    return mecanico
...
def update_mecanico(mecanico: Mecanico):
    ...
    params = (
        ...
        mecanico.email, mecanico.ativo, mecanico.id_usuario
    )
    execute_command(query, params)
    logger.info("mecânico atualizado id=%s", mecanico.id_usuario)
    return mecanico
...
def soft_delete_mecanico(mecanico: Mecanico):
    ...
    WHERE id_usuario = %s
    """
    execute_command(query, (mecanico.id_usuario,))
    logger.info("mecânico soft-delete id=%s", mecanico.id_usuario)
    return mecanico
...
def check_email_exists(email: str, exclude_id: int = None):
    ...
    if exclude_id:
        ...
        FROM mecanico 
        WHERE email = %s AND id_usuario != %s AND deleted_at IS NULL
        """
