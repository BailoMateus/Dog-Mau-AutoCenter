...
@router.post("/{user_id}/foto-perfil", response_model=UserPublic)
@router.post("/{user_id}/upload-photo", response_model=UserPublic, include_in_schema=False)
def upload_user_photo(
    user_id: Annotated[int, Path(ge=1)],
    file: UploadFile = File(...),
    current=Depends(get_current_user),
):
    logger.info("POST /users/%s/foto-perfil actor=%s", user_id, current.get("user_id"))
