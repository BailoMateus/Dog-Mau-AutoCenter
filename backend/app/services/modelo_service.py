...
def list_modelos():
    return repo.get_all_modelos()

def list_modelos_by_marca(marca_id: int):
    return repo.get_modelos_by_marca(marca_id)

def create_modelo(data: ModeloCreate):
    modelo = Modelo(
    ...
    return repo.create_modelo(modelo)

def get_modelo_or_404(modelo_id: int) -> Modelo:
    modelo = repo.get_modelo_by_id(modelo_id)
    if not modelo:
...
def update_modelo(modelo_id: int, data: ModeloUpdate):
    modelo = get_modelo_or_404(modelo_id)
    ...
    return repo.update_modelo(modelo)

def delete_modelo(modelo_id: int):
    modelo = get_modelo_or_404(modelo_id)
    return repo.soft_delete_modelo(modelo)